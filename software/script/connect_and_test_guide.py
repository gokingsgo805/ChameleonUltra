...
def main():
    ...
    print("  • Card type is detected (EM4100, T55xx, MIFARE, etc)")
    print("  • Available commands are displayed")
    print("  • Device info and capabilities are shown\n")

    print(f"{CG}Expected output:{C0}")
    print("  [OK] Device searched")
    print("  [OK] Chameleon Ultra connected: v3.0")
    print("  [OK] Card Detected: EM4100 (LF - 125 kHz)")
    print("    Type: EM4100 RFID Tag")
    print("    Frequency: 125 kHz")
    print("  [OK] Available Commands")
    print("    - lf em 410x read")
    print("    - lf em 410x clone")
    print("    - lf t55xx chk\n")

    print(f"{CG}STEP 2: Test with Fuzzer{C0}\n")

    print("Once connected, test your card's robustness:")
    ...
    print("Test different mutation strategies:")
    ...
    print("  lf search        - Manual card search")
    print("  lf em 410x info  - Get EM4100 details")
    print("  lf t55xx info    - Get T55xx details\n")

    print(f"{CY}EM4100 Operations:{C0}")
    print("  lf em 410x read  - Read card ID")
    print("  lf em 410x clone - Clone to Chameleon")
    print("  lf em 410x sim   - Simulate card\n")

    print(f"{CY}T55xx Operations:{C0}")
    print("  lf t55xx chk --dic common_t55xx_passwords.dic")
    print("  lf t55xx clone --pwd 00000000\n")

    print(f"{CY}Fuzzer Operations:{C0}")
    print("  hf mf fuzz -i <iterations>")
    print("  hf mf fuzz -m <mutation_type>  (bit, byte, xor, random)")
    print("  hf mf fuzz --block <block_num>")
    print("  hf mf fuzz --health-check <interval>\n")

    print(f"{CB}{'=' * 70}{C0}")
    print(f"{CB}TIPS FOR BEST RESULTS{C0}")
    print(f"{CB}{'=' * 70}{C0}\n")

    print("[OK] Keep card on antenna during entire test")
    print("[OK] Use health checks for longer tests (--health-check 50)")
    print("[OK] Start with small iterations, increase gradually")
    print("[OK] Test multiple mutation strategies")
    print("[OK] Save results for documentation (--output file.csv)")
    print("[OK] Use verbose mode to see device details (--verbose)")
    print("[OK] Try T55xx password check before fuzzing")
    print("[OK] Monitor card temperature during extended testing\n")
    ...
    print("   -> Device connects, card auto-detected")
    print(f"2. {CG}hf mf fuzz -i 100 -m bit{C0}")
    print("   -> Run quick vulnerability test")
    print(f"3. {CG}hf mf fuzz -i 500 -o results.csv{C0}")
    print("   -> Run extended test, save results\n")
