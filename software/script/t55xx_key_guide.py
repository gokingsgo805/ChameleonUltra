...
def main():
    ...
    print(f"  EM4100:  Read-only LF tag (no keys needed)")
    print(f"  T55xx:   LF chip WITH password protection (needs T55xx dict)")
    print(f"  Connection: EM4100 can EMULATE T55xx with password\n")
    
    print_section("T55xx KEY DICTIONARY FORMAT")
    
    print(f"{CG}T55xx uses 32-bit (8 hex chars) passwords:{C0}")
    print(f"  Format:    8 hexadecimal characters")
    print(f"  Example:   51243648")
    print(f"  Compare:   MIFARE Classic = 12 hex chars (6 bytes)\n")
    
    print(f"{CG}Example .dic file for T55xx:{C0}")
    print(f"  00000000  (all zeros - default)")
    print(f"  FFFFFFFF  (all ones)")
    print(f"  51243648  (common password)")
    print(f"  19920427  (date format)")
    print(f"  12345678  (sequential)\n")
    ...
    print(f"  lf t55xx info          - Get T55xx info")
    print(f"  lf t55xx read          - Read T55xx data")
    print(f"  lf t55xx chk           - Check passwords")
    print(f"  lf t55xx chk --dic X   - Check with dictionary\n")
    
    print(f"{CG}Using with dictionary:${C0}")
    print(f"  lf t55xx chk --dic t55xx_keys.dic")
    print(f"  lf t55xx chk --dic common_t55xx_passwords.dic\n")
    
    print(f"{CG}Clone/Write operations:${C0}")
    print(f"  lf t55xx clone --pwd <password>")
    print(f"  lf t55xx write --pwd <password> <data>\n")
    
    print(f"{CG}Detect/Detect password:${C0}")
    print(f"  lf t55xx detect")
    print(f"  lf t55xx dump")
    print(f"  lf t55xx recover\n")
    ...
    print(f"  {CB}─────────────────┼───────────────┼──────────────{C0}")
    print(f"  Read-only         │ Yes           │ No")
    print(f"  Has password      │ No (inherent) │ Yes")
    print(f"  Key dict needed   │ No            │ Yes (T55xx)")
    print(f"  Key size          │ N/A           │ 32-bit (8 hex)")
    print(f"  Commands          │ lf em 410x    │ lf t55xx\n")
    ...
    print(f"   You can then clone/modify the card\n")
